...
function MeetingRoom({ user }) {
  ...
  const [agendaInteractions, setAgendaInteractions] = useState({});
  ...
  useEffect(() => {
    if (messages && messages.length > 0) {
      ...
      // Map message types to agenda item IDs
      const messageTypeToAgendaId = {
        'CALL_TO_ORDER': 1,
        'READING_OF_MINUTES': 2,
        'BUDGET_APPROVAL': 3,
        'MOTION_AND_VOTING': 4,
        'COMMITTEE_REPORTS': 5,
        'NEW_BUSINESS': 6,
        'ADJOURNMENT': 7,
        'POST_MEETING': 8,
        'AGENDA_ITEM_CLICK': null // Generic fallback
      };
      
      const agendaItemId = messageTypeToAgendaId[latestMessage.type];
      
      // Handle agenda item interaction messages
      if (agendaItemId && latestMessage.data) {
        const { from, userId, Role, text } = latestMessage.data;
        
        setAgendaInteractions(prev => {
          const itemInteractions = prev[agendaItemId] || [];
          const existingInteraction = itemInteractions.find(i => i.userId === userId);
  ...
  // Handler for agenda item clicks
  const handleAgendaItemClick = (item) => {
    setSelectedAgendaItem(item);
    setActiveTab('history');
    
    // Send WebSocket message for the clicked agenda item
    if (isConnected) {
      // Map agenda item IDs to message types for backend
      const messageTypeMap = {
        1: 'CALL_TO_ORDER',
        2: 'READING_OF_MINUTES',
        3: 'BUDGET_APPROVAL',
        4: 'MOTION_AND_VOTING',
        5: 'COMMITTEE_REPORTS',
        6: 'NEW_BUSINESS',
      ...
      const messageType = messageTypeMap[item.id] || 'AGENDA_ITEM_CLICK';
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className={`fixed right-0 top-0 h-full w-80 bg-white border-l border-gray-200 shadow-lg transform transition-transform duration-300 ${
          ...
          <div className="flex-1 overflow-y-auto p-4">
            ...
            {activeTab === 'history' && (
              <div className="space-y-3">
                <h3 className="text-sm font-bold text-gray-900 mb-3">📜 AGENDA ITEM DETAILS</h3>
                {selectedAgendaItem ? (
                  <div className="space-y-4">
                    <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                      <h4 className="text-sm font-bold text-gray-900 mb-2">
                        {selectedAgendaItem.id}. {selectedAgendaItem.title}
                      </h4>
                      <div className="space-y-2 text-xs text-gray-700">
                        <p><strong>Status:</strong> <span className="capitalize">{selectedAgendaItem.status}</span></p>
                        {selectedAgendaItem.duration && (
                          <p><strong>Estimated Duration:</strong> {selectedAgendaItem.duration}</p>
                        )}
                      </div>
                    </div>

                    <div className="p-4 bg-white border border-gray-200 rounded-lg">
                      <h5 className="text-xs font-bold text-gray-900 mb-2">📝 Description</h5>
                      <p className="text-xs text-gray-600">
                        {selectedAgendaItem.id === 1 && "The meeting begins with the chairman calling the meeting to order and welcoming all participants."}
                        {selectedAgendaItem.id === 2 && "Review and approval of minutes from the previous meeting. Members may propose corrections or amendments."}
                        {selectedAgendaItem.id === 3 && "Discussion and voting on the proposed Q4 budget allocation. Includes review of financial reports and projected expenses."}
                        {selectedAgendaItem.id === 4 && "Formal motion presentation and voting procedures. Members will have the opportunity to propose, second, and vote on motions."}
                        {selectedAgendaItem.id === 5 && "Standing committees present their reports and recommendations for board consideration."}
                        {selectedAgendaItem.id === 6 && "Discussion of new business items that have been added to the agenda. Members may introduce new topics for consideration."}
                        {selectedAgendaItem.id === 7 && "Formal closing of the meeting by the chairman. Final announcements and scheduling of next meeting."}
                        {selectedAgendaItem.id === 8 && "Post-meeting activities including minutes finalization and action item assignments."}
                      </p>
                    </div>

                    {/* Participant Activity Section */}
                    <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                      <h5 className="text-xs font-bold text-gray-900 mb-3 flex items-center justify-between">
                        <span>👥 Participant Activity</span>
                        <span className="px-2 py-0.5 bg-green-200 text-green-800 rounded-full">
                          {agendaInteractions[selectedAgendaItem.id]?.length || 0}
                        </span>
                      </h5>
                      <div className="space-y-2">
                        {agendaInteractions[selectedAgendaItem.id]?.length > 0 ? (
                          agendaInteractions[selectedAgendaItem.id].map((interaction, index) => (
                            <div 
                              key={index}
                              className="flex items-center gap-3 p-2 bg-white rounded border border-green-100"
                            >
                              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-semibold flex-shrink-0 ${
                                interaction.role === 'chairman' ? 'bg-yellow-500' : 'bg-blue-500'
                              }`}>
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-xs font-medium text-gray-900 truncate">
                                  ...
                                  {interaction.role === 'chairman' && (
                                    <span className="text-yellow-600 text-xs ml-1">👑</span>
                                  )}
                                </p>
                                <p className="text-xs text-gray-500">
                                  {interaction.action === 'viewed' ? '👁️ Viewed' : '✓ Interacted'} • {new Date(interaction.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                </p>
                              </div>
                            </div>
                          ))
                          ...
                          <p className="text-xs text-gray-500 text-center py-2">
                            No participants have viewed this item yet
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="p-4 bg-white border border-gray-200 rounded-lg">
                      <h5 className="text-xs font-bold text-gray-900 mb-2">🗳️ Actions & Motions</h5>
                      <div className="space-y-2">
                        <p className="text-xs text-gray-500">No motions recorded yet for this agenda item.</p>
                      </div>
                    </div>

                    <div className="p-4 bg-white border border-gray-200 rounded-lg">
                      <h5 className="text-xs font-bold text-gray-900 mb-2">💬 Discussion Summary</h5>
                      <p className="text-xs text-gray-500">Discussion has not started yet.</p>
                    </div>

                    <button
                      onClick={() => setActiveTab('agenda')}
                      className="w-full px-3 py-2 bg-gray-100 text-gray-700 text-xs rounded hover:bg-gray-200 transition-colors"
                    >
                      ← Back to Agenda
                    </button>
                  </div>
                  ...
                  <div className="text-center py-8">
                    <p className="text-sm text-gray-400 mb-3">Select an agenda item to view details</p>
                    <button
                      onClick={() => setActiveTab('agenda')}
